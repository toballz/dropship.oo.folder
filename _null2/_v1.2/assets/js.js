...
document.querySelector(".continyeto_pickadate").onclick = function(){
    ...
    setTimeout(function(){
        ...
        if(t==0){alert("Please Select a HairStyle From the Collection.");loader.stop();};
...
function saveContactInfo(){
    ...
    setTimeout(function(){
        ...
        if (fnm.val().trim().indexOf(' ') < 0) { err="Please type your First and Last name!"; }
        if (eml.val().trim().indexOf('@') < 0) { err="Please Input a valid Email!"; } 
        if (pnb.val().trim().length > 13 || pnb.val().trim().length < 4) {err="Please Input a valid Phone number!"; }

        if(err){
            alert(err);loader.stop();
        }else{
            if(dateSelecta !== null && ttimeesele !== null && hairstyle !== null){
                ...
                alert("Error please reload this page.");loader.stop();
            }
        }
    },1000);
